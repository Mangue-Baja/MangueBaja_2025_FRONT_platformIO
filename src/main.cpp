#include "mbed.h"
#include "Other_things/stats_report.h"
#include <Serial.h>
/* Instances Libraries */
#include "CANMsg.h"
#include <LSM6DS3.h>
...
LSM6DS3 LSM6DS3_IMU(PB_7, PB_6);        // SDA, SCL
...
int main ()
{
    ...
    uint16_t lsm_addr = LSM6DS3_IMU.begin(LSM6DS3_IMU.G_SCALE_245DPS, LSM6DS3_IMU.A_SCALE_2G, \
                                      LSM6DS3_IMU.G_ODR_26_BW_2, LSM6DS3_IMU.A_ODR_26); 
    setupInterrupts();
    ...
    while (true)
    {
        ...
        switch(current_state) 
        {
            ...
            case IMU_ST:
                ...
                if (lsm_addr)
                {
                    bool nack = LSM6DS3_IMU.readAccel();     // read accelerometer data into LSM6DS3_IMU.aN_raw
                    if (!nack)
                        nack = LSM6DS3_IMU.readGyro();       //  "   gyroscope data into LSM6DS3_IMU.gN_raw
                    
                    if (nack)
                    {
                        lsm_addr = 0;
                        LSM6DS3_IMU.ax_raw = 0;
                        LSM6DS3_IMU.ay_raw = 0;
                        LSM6DS3_IMU.az_raw = 0;
                        LSM6DS3_IMU.gx_raw = 0;
                        LSM6DS3_IMU.gy_raw = 0;
                        LSM6DS3_IMU.gz_raw = 0;
                ...
                else if (imu_failed == IMU_TRIES)
                {
                    ...
                    lsm_addr = LSM6DS3_IMU.begin(LSM6DS3_IMU.G_SCALE_245DPS, LSM6DS3_IMU.A_SCALE_2G, LSM6DS3_IMU.G_ODR_26_BW_2, LSM6DS3_IMU.A_ODR_26);                                    
                    //t1 = t.read_us();
                    imu_failed = 0;
                    //serial.printf("%d\r\n", (t1 - t0));
                } 
                ...
                last_acq = t.read_ms();
                calcAngles(LSM6DS3_IMU.ax_raw, LSM6DS3_IMU.ay_raw, LSM6DS3_IMU.az_raw, LSM6DS3_IMU.gx_raw, LSM6DS3_IMU.gy_raw, LSM6DS3_IMU.gz_raw, dt);


                /* Send accelerometer data */
                txMsg.clear(IMU_ACC_ID);
                txMsg << LSM6DS3_IMU.ax_raw << LSM6DS3_IMU.ay_raw << LSM6DS3_IMU.az_raw;
                if (can.write(txMsg))
                {
                    ...
                    txMsg << LSM6DS3_IMU.gx_raw << LSM6DS3_IMU.gx_raw << LSM6DS3_IMU.gx_raw /*<< dt*/;
                    if (can.write(txMsg))
                    {
